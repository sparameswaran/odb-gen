...
with open("./manifest.json", "r") as myfile:
    existing_manifest = myfile.read().replace('\n', ' ').replace('\r', '')

with open("./plan.json", "r") as myfile:
    plan = myfile.read().replace('\n', ' ').replace('\r', '')

with open("./request2.json", "r") as myfile:
    requestParams = myfile.read().replace('\n', ' ').replace('\r', '')

#with open("./service-releases.json", "r") as myfile:
#    service_releases = myfile.read().replace('\n', ' ').replace('\r', '')
call(['go', 'run', '../src/{{product['short_name']}}-service-adapter/cmd/service-adapter/main.go', argv[1], bosh_info, plan, requestParams, existing_manifest, '{}'])
